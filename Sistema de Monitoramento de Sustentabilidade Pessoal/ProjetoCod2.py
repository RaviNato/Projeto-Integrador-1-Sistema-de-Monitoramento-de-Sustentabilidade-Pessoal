...
def listaDeDados():
    comando = f"SELECT codigo, nome, DATE_FORMAT(data_checagem,'%d/%m/%Y'), agua_litros, energia_KWh, residuos_nao_resiclaveis_Kg, residuos_reciclados, transporte FROM sustentabilidade;"
    conexao=obterConxao()
    cursor=conexao.cursor()
    cursor.execute(comando)
...
def alterarBD(coluna, codigo, novoBD, novoBD2, novoBD3):
    if coluna == "data_checagem":
        comando = f"UPDATE sustentabilidade SET data_checagem = '{novoBD3}.{novoBD2}.{novoBD}' WHERE codigo = '{codigo}';"
    else:
        comando = f"UPDATE sustentabilidade SET {coluna} = '{novoBD}' WHERE codigo = '{codigo}';"
    conexao=obterConxao()
    cursor=conexao.cursor()
    cursor.execute(comando)
    conexao.commit()
...
def checagemNao(codigo):
    comando = f"SELECT * FROM sustentabilidade WHERE codigo = '{codigo}';"
    conexao=obterConxao()
    cursor=conexao.cursor()
    cursor.execute(comando)
...
def deletar(codigo):
    try:
        ...
        if codigoN:
            ...
            comando = f"DELETE FROM sustentabilidade WHERE codigo = {codigo};"
            conexao=obterConxao()
            cursor=conexao.cursor()
            cursor.execute(comando)
            conexao.commit()
